...
public class Task_9_1 {
	...
	public void map(LongWritable cur_key, Text cur_val, Context cur_out) throws IOException, InterruptedException {
	    String cur_lne = cur_val.toString();//string of current line
	    String[] cur_lst = cur_lne.split(",",0);//array of seperated words of the current line
	    String[] cur_wta = cur_lst[13]; // the country that the winner belongs to
            String country = "NA";//set default country name to "NA"
	    if (cur_wta.length == 3 ) {	    
                  country = cur_wta[3];
                  //replace the country name if the length of the cur_wta >3
            }
	    String winner = cur_lst[7];
           
	    //We now have country and winner, let's emit
        // the tuple we emit:((winner_id, country),1)
	    String emt_str = winner + "," + country+ ",";
	    if ((emt_str.equals("") == false ) && (emt_str.length() > 3)) {//check if the emt_str is not empty or greater than 3
	      emt_wrd.set(emt_str);//set emt_wrd to emt_str
	...
	job.setJarByClass(Task_9_1.class);
